...
const EmailInput: React.FC<Props> = ({email_value, setEmail_value, setErrors_array}) => {
    ...
    const onEmailChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const user = await checkEmailAvailable(value);

            if (user) {
                setErrors_array(prev => [...new Set([...prev, 'This email address already exists in the system'])]);
            } else {
                setErrors_array(prev => [...prev.filter(item => item !== 'This email address already exists in the system')]);
            }
